...
@interface MyProjectMaterialVC () <UITableViewDelegate,UITableViewDataSource>
...
@implementation MyProjectMaterialVC
...
- (void)loadData {
    ...
    NSString *des = [DES encryptUseDES:[[NSString stringWithFormat:@"action==xmxx/material/tree.do?xmxxId=%@&applicationId=1&isFilter=1&pid=%@&isContractAgent=是,%@",_listModel.ID,_listModel.PID,ticket] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] key:Key];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    fileImage.image = [UIImage imageNamed:@"fileicon"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 附件(材料)下载
    NSString *ticket = [UserDefaults objectForKey:@"ticket"];
    NSString *des = [DES encryptUseDES:[[NSString stringWithFormat:@"action==xmxx/material/download.do?mid=%@&xmxxId=%@&tid=%@,%@",model.realId,_listModel.ID,_listModel.TASK_ID,ticket] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] key:Key];
