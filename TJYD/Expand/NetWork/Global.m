...
+(void)initializeSystem:(NSString *)serverAddress deviceUUID:(NSString *)deviceUUID Cookie:(NSString *)Cookie user:(NSDictionary *)userInfo{
    ...
    [UserDefaults setObject:_deviceUUID forKey:@"deviceUUID"];
    _Cookie = Cookie;
    [UserDefaults setObject:_Cookie forKey:@"Cookie"];
    _userInfo = userInfo;
//    NSUserDefaults *accountDefaults = [NSUserDefaults standardUserDefaults];
//    [accountDefaults setObject:userInfo forKey:@""];
    [UserDefaults setObject:userInfo forKey:@"account"];
    [Global setUserId:[_userInfo objectForKey:@"id"]];
    [UserDefaults setObject:[userInfo objectForKey:@"name"] forKey:@"name"];
...
+ (NSString *)BpmSSOApi {
    ...
    return @"http://116.236.160.182:9083";//公司外网
    //return @"http://172.30.240.63:9080";//同济现场正式环境
    //return @"http://10.194.150.2:8080";//同济现场测试环境
}

// 表单
+ (NSString *)BpmFormApi {
    return @"http://116.236.160.182:9081";//公司外网
    //return @"http://172.30.240.38:9081";//同济现场正式环境
    //return @"http://10.194.150.3:9081";//同济现场测试环境
}

// 前端页面
+ (NSString *)BpmProxyApi {
//    return @"http://116.236.160.182:9081/DAP4Resource/TJProxyWebView.html";//公司外网
    return @"http://116.236.160.182:9083/AppCenter/dist/views/proxy.html";//前端服务器
    //return @"http://172.30.240.38:9081/DAP4Resource/GZproxyWKWeb.html";//同济现场正式环境
    //return @"http://10.194.150.3:9081/DAP4Resource/GZproxyWKWeb.html";//同济现场测试环境
}
