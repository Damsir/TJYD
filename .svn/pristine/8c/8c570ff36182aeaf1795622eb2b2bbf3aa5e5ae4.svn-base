...
@interface DistRequest () <DistChainRequestDelegate>
...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        ...
        [DistNetworkPrivate addDoNotBackupAttribute:path];
...
- (NSString *)cacheBasePath {
    ...
    NSArray *filters = [[DistNetworkConfig sharedInstance] cacheDirPathFilters];
    if (filters.count > 0) {
        
        for (id<DistCacheDirPathFilterProtocol> f in filters) {
...
- (NSString *)cacheFileName {
    ...
    NSString *baseUrl = [DistNetworkConfig sharedInstance].baseUrl;
    ...
    NSString *requestInfo = [NSString stringWithFormat:@"Method:%ld Host:%@ Url:%@ Argument:%@ AppVersion:%@ Sensitive:%@",
                             ...
                             argument, [DistNetworkPrivate appVersionString], [self cacheSensitiveData]];
    NSString *cacheFileName = [DistNetworkPrivate md5StringFromString:requestInfo];
...
- (NSDictionary *)requestHeaderFieldValueDictionary {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"HTTPHEAD"]) {
        
        return [NSDictionary dictionaryWithObjectsAndKeys:[[NSUserDefaults standardUserDefaults] objectForKey:@"HTTPHEAD"],@"User-Agent", nil];
