...
//    [_searchResultTabelView.header setAutoChangeAlpha:YES];
//    [_searchResultTabelView.header beginRefreshing];
}
...
- (void)viewDidLoad {
    ...
    [self initNavigationBarTitle:@"综合查询"];
    //初始化导航栏;
    [self initNav];
    //创建是搜索栏
    self.searchBar  = [[DY_newSearchBar alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 50)];
...
- (void)changeComS
{
    ...
    self.historyTabelView.frame=CGRectMake(0, self.searchBar.frame.size.height, SCREEN_WIDTH, (SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2);
    if (self.searchBar.frame.size.height+self.historyTabelView.historyArray.count*40+30+44 <= (SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2)
    {
        self.searchResultTabelView.frame = CGRectMake(0,self.searchBar.frame.size.height+self.historyTabelView.historyArray.count*40+30+44, SCREEN_WIDTH,SCREEN_HEIGHT-self.searchBar.frame.size.height-64-(self.historyTabelView.historyArray.count*40+30+44));
        _loadBtn.frame =CGRectMake((SCREEN_WIDTH-150)*0.5, self.searchBar.frame.size.height+self.historyTabelView.historyArray.count*40+30+44+20, 150, 200);
    }
    else
    {
        self.searchResultTabelView.frame = CGRectMake(0,(SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2+self.searchBar.frame.size.height, SCREEN_WIDTH,SCREEN_HEIGHT-(SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2-64-self.searchBar.frame.size.height);
        _loadBtn.frame =CGRectMake((SCREEN_WIDTH-150)*0.5, (SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2+self.searchBar.frame.size.height+20, 150, 200);
    }
    _tipLable.frame= CGRectMake((SCREEN_WIDTH-150-30)*0.5,CGRectGetMaxY(_loadBtn.frame)-44, _loadBtn.frame.size.width+30,44);
...
{
    self.historyTabelView =[[DY_historyTableView alloc] initWithFrame:CGRectMake(0, self.searchBar.frame.size.height, SCREEN_WIDTH, (SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2)];
...
- (void)createWithResultTableView{
    //439
    self.searchResultTabelView = [[UITableView alloc] initWithFrame:CGRectMake(0,self.searchBar.frame.size.height+self.historyTabelView.historyArray.count*40+30+44, SCREEN_WIDTH,SCREEN_HEIGHT-self.searchBar.frame.size.height-64-(self.historyTabelView.historyArray.count*40+30+44)) style:UITableViewStylePlain];
    self.searchResultTabelView.hidden = NO;
...
//    sBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
...
- (void)showWithoutDataAndHiden:(BOOL)hiden
{
    ...
    if (!_loadBtn) {
        ...
        //loadBtn.frame =CGRectMake((SCREEN_WIDTH-150)*0.5, (SCREEN_HEIGHT-200)*0.5-50, 150, 200);
        loadBtn.frame =CGRectMake((SCREEN_WIDTH-150)*0.5, self.searchBar.frame.size.height+self.historyTabelView.historyArray.count*40+30+44+20, 150, 200);
    ...
    if (!_tipLable) {
       ...
       // tipLabel.frame= CGRectMake((SCREEN_WIDTH-150-30)*0.5,CGRectGetMaxY(_loadBtn.frame)-44, _loadBtn.frame.size.width+30,44);
        tipLabel.frame= CGRectMake((SCREEN_WIDTH-150-30)*0.5,CGRectGetMaxY(_loadBtn.frame)-44, _loadBtn.frame.size.width+30,44);
...
{
//    if (self.historyTabelView.frame<= (SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2)
//    {
//        self.historyTabelView.frame=CGRectMake(0, self.searchBar.frame.size.height, SCREEN_WIDTH, (self.historyTabelView.historyArray.count)*40+30+44);
//    }
//    else
//    {
        self.historyTabelView.frame=CGRectMake(0, self.searchBar.frame.size.height, SCREEN_WIDTH, (SCREEN_HEIGHT-64-self.searchBar.frame.size.height)/2);
