...
-(instancetype)initWithFrame:(CGRect)frame WithControllerViewArray:(NSArray *)viewsArray AndWithTitlesArray:(NSArray *)titlesArray
{
    ...
    if (self)
    {
        ...
        [self initSlideSegmentAndScrollView:frame];
...
- (void)screenRotationWithFrame:(CGRect)frame
{
    ...
    // 和字体宽度一样宽的下划线
    _underLineView.frame = CGRectMake(X,HeaderHeight-2.5,titleSize.width,2);
    // 和按钮宽度一样的下划线
...
- (void)initSlideSegmentAndScrollView:(CGRect)frame {
...
//    // 和字体宽度一样宽的下划线
//     UIView *underLineView = [[UIView alloc] initWithFrame:CGRectMake((segmentWidth - titleSize.width)*0.5,HeaderHeight-2.5,titleSize.width,2)];
    // 和按钮宽度一样宽的下划线
...
//        [_scrollView setContentOffset:CGPointMake(SCREEN_WIDTH * indexPath.row, 0) animated:YES];
//    }];
